...
_create_appimage() {
	...
	mkdir -p "$APP-$TOOLKIT".AppDir/distribution
	cat <<-'HEREDOC' >> "$APP-$TOOLKIT".AppDir/distribution/policies.json
	{
	  "policies": {
	...
	# Extract the archive
	[ -e ./*tar.* ] && tar fx ./*tar.* && mv ./"$APP"/* "$APP-$TOOLKIT".AppDir/ && rm -f ./*tar.* || exit 1

	# Enter the AppDir
	cd "$APP-$TOOLKIT".AppDir || exit 1

	# Add the launcher and patch it depending on the release channel
	echo "$LAUNCHER" > "$APP".desktop

	# Add the icon
	cp ./browser/chrome/icons/default/default48.png "$APP".png
	cd .. || exit 1

	# Check the version
	VERSION=$(cat ./"$APP-$TOOLKIT".AppDir/application.ini | grep "^Version=" | head -1 | cut -c 9-)

	# Create te AppRun
	cat <<-'HEREDOC' >> ./"$APP-$TOOLKIT".AppDir/AppRun
	#!/bin/sh
	HERE="$(cd "${0%/*}" && echo "$PWD")"
...
toolkits="gtk2 gtk3"
for t in $toolkits; do
	TOOLKIT="$t"
	mkdir -p "$TOOLKIT" && cd "$TOOLKIT" || exit 1
	_create_appimage
	cd .. || exit 1
	mv ./"$TOOLKIT"/*.AppImage* ./
done
